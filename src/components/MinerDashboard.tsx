    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Connecting to GekkoScience CompacF...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">GekkoScience CompacF</h1>
              <p className="text-sm text-gray-600 mt-1">Bitcoin USB Miner Dashboard</p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <StatCard
            title="Hashrate (5s)"
            value={summary?.mhs5s || 0}
            unit="MH/s"
            icon={<Activity className="w-6 h-6" />}
            subtitle={`Avg: ${(summary?.mhsAv || 0).toFixed(2)} MH/s`}
          />
          <StatCard
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{(summary?.mhs5s || 0).toFixed(2)} MH/s</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">1 Minute</span>
                <span className="font-medium">{(summary?.mhs1m || 0).toFixed(2)} MH/s</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">5 Minute</span>
                <span className="font-medium">{(summary?.mhs5m || 0).toFixed(2)} MH/s</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">15 Minute</span>
                <span className="font-medium">{(summary?.mhs15m || 0).toFixed(2)} MH/s</span>
              </div>
            </div>
          </div>
